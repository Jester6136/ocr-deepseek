#!/usr/bin/env python3
"""
PDF to OCR Processor (Enhanced)

This application scans the /data folder for PDF files and converts them to plain text format
...
class PDFToOCRProcessor:
    """Processor for converting PDF files to plain text using DeepSeek OCR API with enhanced post-processing"""
    
    def __init__(self, data_folder: str = "data", api_base_url: str = "http://localhost:8000", 
                 extract_images: bool = True, create_images_folder: bool = True, max_workers: Optional[int] = None):
        """
    ...
    def _call_ocr_api(self, pdf_path: str) -> Optional[str]:
        """
        Call the OCR API to process a PDF file using the OCR prompt
        ...
        try:
            ...
            with open(pdf_path, 'rb') as pdf_file:
                ...
                # Use OCR-specific prompt for this request
                # From config.py: PROMPT = '<image>\nFree OCR.'
                ocr_prompt = '<image>\nFree OCR.'
    ...
    def convert_pdf_to_ocr(self, pdf_path: str) -> Optional[str]:
        """
        Convert a single PDF file to OCR text
        ...
        try:
            ...
            # Save markdown file with -OCR suffix
            pdf_path_obj = Path(pdf_path)
            markdown_path = pdf_path_obj.with_name(f"{pdf_path_obj.stem}-OCR.md")
    ...
    def scan_and_process_all_pdfs(self) -> List[str]:
        """
        Scan the data folder for PDF files and convert all of them to OCR text
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all tasks
            future_to_pdf = {
                executor.submit(self.convert_pdf_to_ocr, str(pdf_file)): pdf_file 
...
def main():
    ...
    print(f"{Colors.BLUE}PDF to OCR Processor (Enhanced){Colors.RESET}")
    ...
    try:
        ...
        if markdown_files:
            print(f"\n{Colors.GREEN}Successfully converted {len(markdown_files)} PDF files to OCR text:{Colors.RESET}")
